...
int
cmapRvecToSt(float rvec[3], float st[2])
{
    double m, recipm;
    ...
    /* In Section 2.10.4 ("Generating texture coordinates")
       ...
       The equation for calculating (rx, ry, rz) is the equation for
       calculating the reflection vector for a surface and observer.
       The explanation and derivation for this equation is found in
       Roger's "Procedural Elements for Computer Graphics" 2nd ed.
       in Section 5-5 ("Determining the Reflection Vector").  Note that
       Roger's convention has the Z axis in the opposite direction from
...
void
cmapStToRvec(float *st, float *rvec)
{
    double tmp1, tmp2;

    /* Using algebra to invert the sphere mapping equations
       shown above in cmapRvecToSt, you get:
